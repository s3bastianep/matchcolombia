import React from "react";
import { Check } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * Badge exclusivo mint/turquesa — solo para confianza / verificado.
 * No reutilizar este color para otros estados o decoración.
 */
export default function VerifiedBadge({ score, className, size = "sm" }) {
  return (
    <span
      className={cn(
        "badge-verified",
        size === "xs" && "text-[9px] px-2 py-0.5 gap-0.5",
        size === "md" && "text-xs px-3 py-1 gap-1.5",
        className
      )}
    >
      <Check className={cn("shrink-0", size === "md" ? "w-3.5 h-3.5" : "w-3 h-3")} strokeWidth={3} />
      <span>Verificado</span>
      {score != null && score > 0 && (
        <span className="font-semibold opacity-80 tabular-nums">{score}%</span>
      )}
    </span>
  );
}
